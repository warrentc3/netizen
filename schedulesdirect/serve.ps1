...
try {
    while ($listener.IsListening) {
        ...
        # Proxy /api/* to Schedules Direct
        if ($relativePath.StartsWith('api/')) {
            $sdPath = $relativePath.Substring(4)
            $sdUrl = "https://json.schedulesdirect.org/20141201/$sdPath"
            $query = $request.Url.Query
            if ($query) { $sdUrl += $query }

            try {
                $proxyHeaders = @{}
                foreach ($key in $request.Headers.AllKeys) {
                    if ($key -in 'Host', 'Connection', 'Accept-Encoding') { continue }
                    $proxyHeaders[$key] = $request.Headers[$key]
                }
                ...
                $proxyResp = Invoke-WebRequest @iwrParams -ErrorAction Stop
                $response.StatusCode = $proxyResp.StatusCode
                $response.ContentType = $proxyResp.Headers['Content-Type']
                $respBytes = $proxyResp.Content
                if ($respBytes -is [string]) {
                    $respBytes = [System.Text.Encoding]::UTF8.GetBytes($respBytes)
                }
                ...
                $errMsg = [System.Text.Encoding]::UTF8.GetBytes("Proxy error: $_")
